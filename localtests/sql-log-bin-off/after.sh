...
local replica_server_uuid gtid_executed gtid_purged
...
if echo "$gtid_executed" | grep -q "$replica_server_uuid" ; then
  echo
  echo "ERROR gtid_executed contains gh-ost gtids"
...
max_pristine_master_tx=$(echo "$gtid_executed_pristine" | cut -d ":" -f 2 | cut -d "-" -f 2)
max_current_master_tx=$(echo "$gtid_executed" | cut -d ":" -f 2 | cut -d "-" -f 2)
