...
func TestParseCheckpointCoordinatesUsesMariaDBFlavor(t *testing.T) {
	migrationContext := base.NewMigrationContext()
	migrationContext.UseGTIDs = true
	migrationContext.ApplierMySQLVersion = "10.6.18-MariaDB-log"

	coordinates, err := NewApplier(migrationContext).parseCheckpointCoordinates("0-1-100")
	require.NoError(t, err)
	require.Equal(t, "0-1-100", coordinates.String())
	_, ok := coordinates.(*mysql.GTIDBinlogCoordinates)
	require.True(t, ok)
}
